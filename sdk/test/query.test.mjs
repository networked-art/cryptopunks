...
describe('compileOfferSlot — text-search free terms', () => {
  ...
  it('rejects two multi-match any-of groups in one filter', () => {
    /// Both `hair` (many Accessory matches) and the skin-tone-induced head
    /// any-of group from `albino` need the filter's single any-of slot.
    assert.throws(
      () => compileOfferSlot(data, { query: { text: 'albino hair' } }),
      /any-of trait group/,
    )
  })
})
