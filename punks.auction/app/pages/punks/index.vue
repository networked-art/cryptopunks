...
const description = computed(() => searchOgDescription(query.value))
const titleFor = (suffix: string) =>
  query.value ? `${query.value} · ${suffix}` : `Browse Punks · ${suffix}`
...
if (ogSearch.value) {
  defineOgImage('Search', {
    gridSrc: searchGridSrc(ogSearch.value.label, sale.value),
  })
}
