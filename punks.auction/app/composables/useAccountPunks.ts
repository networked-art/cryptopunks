 ...
 * Yuga `Stash`. Returns a deduplicated sorted union.
 *
...
export function useAccountPunks(opts: {
  ...
  async function load() {
    ...
    // never shows A's grid under B's header.
    reset()
    if (!acct) {
      loading.value = false
      error.value = null
    ...
    try {
      ...
      for (const row of punks) v2Set.add(Number(row.punk_id))
      const v1Set = new Set<number>()
      for (const row of v1Punks) v1Set.add(Number(row.punk_id))
  ...
  return { ids, v1Ids, v2Ids, loading, error, refresh: load }
}
