...
describe('PunksCollectionBids', () => {
  ...
  describe('adjustBidSettlementPrice', () => {
    it('lets the bidder change the settlement reward up and down to zero', async () => {
      const ctx = await deployCollectionBidsStack()
      const { bids, bidder } = ctx
      const bidId = await placeBid(ctx, bidder, {
        bidWei: parseEther('1'),
      ...
      await bidsAsBidder.write.adjustBidSettlementPrice(
        [bidId, parseEther('0.02'), true],
        { value: parseEther('0.02') },
      )
      let [, settlementWei] = (await bids.read.bids([bidId])) as [
        bigint,
        bigint,
        string,
      ]
      assert.equal(settlementWei, parseEther('0.07'))
      ...
      ;[, settlementWei] = (await bids.read.bids([bidId])) as [
        bigint,
        bigint,
        string,
      ]
      assert.equal(settlementWei, 0n)
    })

    it('rejects over-decreasing the settlement reward', async () => {
      const ctx = await deployCollectionBidsStack()
      const { bids, bidder } = ctx
      const bidId = await placeBid(ctx, bidder, {
        bidWei: parseEther('1'),
      ...
      await ctx.viem.assertions.revertWithCustomError(
        bidsAsBidder.write.adjustBidSettlementPrice([
          bidId,
          parseEther('0.06'),
          false,
        ]),
