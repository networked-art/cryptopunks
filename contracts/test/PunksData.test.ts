...
describe('PunksData', () => {
  it('loads, seals, and removes loader authority', async () => {
    ...
    const dataAsOther = await ctx.viem.getContractAt(
      'PunksData',
      data.address,
      {
        client: { wallet: other },
  ...
  it('reads bitmap words across multiple SSTORE2 chunks', async () => {
    ...
    assert.equal(
      await data.read.colorBitmapWord([
        MAX_COLOR_COUNT - 1,
        WORDS_PER_BITMAP - 1,
      ]),
      LAST_CHUNK_PATTERN,
  ...
  it('enforces the full mask combination matrix on hasTraits', async () => {
    ...
    assert.equal(
      await data.read.hasTraits([0, set(0), 0n, set(0) | set(2)]),
      true,
    )
...
async function deployLoadedPunksData(opts?: {
  pixelOverrideForPunk0?: Uint8Array
}) {
...
function makeFixture(opts?: { pixelOverrideForPunk0?: Uint8Array }) {
  ...
  const pixelCountBitmaps = new Uint8Array(
    (332 - 148 + 1) * WORDS_PER_BITMAP * 32,
  )
...
function makeMalformedEntry(opts: {
  ...
  const indexBytesLen =
    Math.ceil((visibleBits.length * bitsPerIndex) / 8) + extraIndexBytes
