...
| 4 | `MAX_LOT_ITEMS = 40`. Gas-bounded so worst-case settlement fits comfortably within L1 mainnet block limits. Subject to confirmation against post-implementation gas measurements. |
| 5 | Every slot and every item names its standard explicitly (`CRYPTOPUNKS` or `CRYPTOPUNKS_V1`). No "any standard" sentinel — order is index-bound, so the bidder is specific. |
